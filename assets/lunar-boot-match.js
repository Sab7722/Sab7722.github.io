...
(function lunarBoot() {
  ...
  function enterPlay(fresh) {
    ...
    try {
      if (fresh) Y.setState({ started: false, found: [], notes: [], cineFresh: false, seenIntro: false });
    } catch {}
    try {
      const g = Y.getState();
  ...
  function playIntro() { enterPlay(true); }
  function startPlay(fresh) { enterPlay(!!fresh); }
