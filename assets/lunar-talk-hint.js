...
(function lunarTalk() {
  ...
  function answer(asked, world, live) {
    ...
    } else if (/what should i|what do i|what next|job|stuck|help me|what now|i'm lost|im lost|where do i|where should/.test(q)) {
      if (typeof window.__laRookHint === "function") {
        text = window.__laRookHint(s, /where/.test(q) ? "where" : /why/.test(q) ? "why" : "stuck");
      } else {
      text = s.job
        ? hale
