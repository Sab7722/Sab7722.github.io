...
(function lunarTalk() {
  ...
  function answer(asked, world, live) {
    ...
    if (/airlock|air lock|\block\b|hatch|eva|cycle|go outside|go out|can't leave|cant leave|won't open|wont open/.test(q)) {
    ...
    } else if (/what should i|what do i|what next|job|stuck|help me|what now|where do i|how do i/.test(q)) {
      let hint = "";
      try { if (typeof window.__laRookHint === "function") hint = window.__laRookHint(s, "stuck") || ""; } catch (err) {}
      text = hint
        ? (hale ? "Right now: " + hint : hint)
        : s.job
          ? hale
      ...
      try { if (typeof window.__laRookHint === "function") hint = window.__laRookHint(s, "stuck") || ""; } catch (err) {}
      text = hint
        ? (hale ? "If you're stuck: " + hint : hint)
        : hale
