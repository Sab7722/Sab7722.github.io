...
(function lunarBoot() {
  ...
  function wipeSave() {
    try {
      localStorage.removeItem("mba-v1");
      localStorage.removeItem("mba-v1.bak");
      sessionStorage.removeItem("mba-v1");
      sessionStorage.removeItem("mba-v1.bak");
    } catch {}
  }

  // New Game / Erase: play the campaign intro. cineFresh stays false so
  // finish/skip lands in the habitat (beginFromStart), not back on the title.
  function startIntro() {
    const now = performance.now();
    if (lastStartAt > 0 && now - lastStartAt < 350) return;
    lastStartAt = now;
    ...
    wipeSave();
    try {
      window.__mbaWatch = false;
      window.__mbaWatchSnap = null;
    } catch {}
    try {
      Y.setState({
        started: false,
    ...
    try {
      const g = Y.getState();
      if (g && typeof g.startCine === "function") g.startCine(false);
    } catch {}
    try {
      Y.setState({
        screen: "intro",
